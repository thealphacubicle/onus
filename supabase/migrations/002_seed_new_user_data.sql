-- Seed fake data for new users (dev only - not for production)
-- Creates a commitment + 4 weeks of sessions, with some checked in, some missed, and penalties
...
CREATE TRIGGER on_auth_user_created_seed
  ...
  FOR EACH ROW EXECUTE FUNCTION public.seed_new_user_data();
